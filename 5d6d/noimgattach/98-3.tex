...
\usepackage[perpage,symbol*]{footmisc}                                                                                 %编号圈1 到圈10
...
\begin{enumerate}[align=left,leftmargin=*,labelindent=0pt,itemindent=!,labelwidth=2.25em,labelsep=0pt,label= \hspace{-0.5em}（\Roman*）]
\item 求$f(x) \)的解析式；
\item 求 在$[0,1]$上的最大值和最小值．
\end{enumerate}
\vfill
...
\begin{enumerate}[align=left,leftmargin=*,labelindent=0pt,itemindent=!,labelwidth=2.5em,labelsep=0pt,label= \hspace{-0.5em}（\Roman*）]
\item 求证：$BD \perp PC$；
\item 求证：$MN\parallel \text{平面}PDC$ ;
\item 求二面角$A-PC-B$的余弦值．
\end{enumerate}
...
\begin{enumerate}[align=left,leftmargin=*,labelindent=0pt,itemindent=!,labelwidth=2.25em,labelsep=0pt,label= \hspace{-0.5em}（\Roman*）]
\item 当$m=1$时，求曲线\(f(x)\)在点$(2,f(2)$ 处的切线方程；
\item 若\(f(x)\)在区间$(2m-1,m+1)$ 上单调递增，求$m$的取值范围．
\end{enumerate}
...
对$\forall \ f(x)\in M$，\begin{enumerate*}[label=\protect\cmcirc{\value*}]\item 方程$f(x)-x=0$有实数根；\item 函数$f(x)$ 的导数 $f'(x)$满足$0<f'(x)<1$ ．\end{enumerate*}
\begin{enumerate}[align=left,leftmargin=*,labelindent=0pt,itemindent=!,labelwidth=2.25em,labelsep=0pt,label= \hspace{-0.5em}（\Roman*）]
\item 判断函数$f(x)=\dfrac x2 +\dfrac {\sin x}4$是否是集合$M$中的元素，并说明理由；
\item 集合$M$中的元素$f(x)$ 具有下面的性质：
